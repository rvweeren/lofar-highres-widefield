...
if os.path.exists(os.getcwd() + 'field_1asec-MFS-image-pb.fits'):
    ...
    CMD = 'wsclean -j {:d} -mem {:d} -data-column {:s} -channels-out {:d} -weight briggs {:s} -store-imaging-weights -taper-gaussian {:s}arcsec -minuv-l 80 -size {:d} {:d} -reorder -weighting-rank-filter 3 -clean-border 0.05 -mgain 0.8 -fit-beam -join-channels -padding 1.25 -auto-mask 2.5 -auto-threshold 1.0 -fit-spectral-pol 3 -pol i -name field_1asec -scale 0.35arcsec -beam-size 1.0 -niter 100000 -use-idg -grid-with-beam -use-differential-lofar-beam -aterm-config aterm-config.txt -idg-mode cpu -parallel-deconvolution 4096 -multiscale -multiscale-scales 0,4,10,25,64,160 -deconvolution-channels 3 -parallel-reordering 6 -temp-dir $PWD *.ms'.format(int(CONFIG['image']['wsclean_ncpu']), int(CONFIG['image']['wsclean_mem']), CONFIG['image']['data_column'], chan_out, CONFIG['image']['robust_full'], CONFIG['image']['taper_full'], int(CONFIG['image']['imsize_full']), int(CONFIG['image']['imsize_full']), MSES[0].split('.')[-1])
